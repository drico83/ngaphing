...
class Dkomponen extends BaseController
{
    ...
    public function getAll()
    {
        ...
        $data['data'] = array();
        $join = "(SELECT ruk.id, sub_komponen.id_prog, sub_komponen.id_rincian, sub_komponen.id_kom, ruk.id_pkm, ruk.id_menu, ruk.status, sum(b.harga_total) as harga_approve FROM ruk JOIN (SELECT `id_sub`, SUM(`harga_total`) as harga_total FROM `usul_belanja` where usul_belanja.tahun='" . $tahun . "' GROUP BY `id_sub`) as b ON b.id_sub=ruk.id JOIN sub_komponen ON sub_komponen.id=ruk.id_menu WHERE ruk.status='Approve' AND sub_komponen.id_kom= '" . $rincian . "' AND ruk.tahun='" . $tahun . "' AND sub_komponen.tahun='" . $tahun . "' GROUP BY ruk.id_pkm) as c";

        $komponen = '(select usul_belanja_rpk.id_pkm, komponen.id, sum(usul_belanja_rpk.harga_total) as harga_total FROM usul_belanja_rpk
        join rpk on rpk.id=usul_belanja_rpk.id_sub join sub_komponen on sub_komponen.id=rpk.id_menu join komponen on komponen.id=sub_komponen.id_kom join rincian on rincian.id=sub_komponen.id_rincian where rpk.status="Approve" AND komponen.id="' . $rincian . '" AND usul_belanja_rpk.tahun="' . $tahun . '" AND komponen.tahun="' . $tahun . '" AND sub_komponen.tahun="' . $tahun . '" group by usul_belanja_rpk.id_pkm) as d';
