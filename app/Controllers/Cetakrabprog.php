...
class Cetakrabprog extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            'controller'        => 'cetakrabprog',
            'title'             => 'Daftar Rencana Usulan Kegiatan ',
            'timer'                => $timer
        ];
        // dd($result);
        return view('cetakrabprog', $data);
    }
    ...
    public function tampil()
    {
        ...
        $rinci = $this->usulanrpkModel->select('usul_belanja_rpk.id_pkm,sum(usul_belanja_rpk.harga_total) as harga_total, upt.pkm')
            ->join('upt', 'usul_belanja_rpk.id_pkm=upt.id')
            ->orderBy('upt.pkm', 'asc')
        ...
        foreach ($rinci as $key => $value) {
            $isitampil .= "<tr>";
            $isitampil .= "<td colspan='18'><b>" . $value->pkm . "</b></td>";
            $isitampil .= "<td style='text-align:right;'><b></b></td>";
            $isitampil .= "</tr>";


            $rincian = $this->usulanrpkModel->select('usul_belanja_rpk.id_pkm,sub_komponen.id_rincian, rincian.nama_rincian, sum(usul_belanja_rpk.harga_total) as harga_total')
                ...
                ->where(['usul_belanja_rpk.id_pkm' => $value->id_pkm, 'sub_komponen.id_rincian' => 8])
            ...
            foreach ($rincian as $key => $value) {
                ...
                $komponen = $this->usulanrpkModel->select('usul_belanja_rpk.id_pkm,sub_komponen.id_kom, komponen.nama_komponen, sum(usul_belanja_rpk.harga_total) as harga_total')
                    ->join('rpk', 'rpk.id=usul_belanja_rpk.id_sub')
                    ->join('sub_komponen', 'sub_komponen.id=rpk.id_menu')
                    ->join('komponen', 'komponen.id=sub_komponen.id_kom')
                    ->join('rincian', 'rincian.id=sub_komponen.id_rincian')
                    ->where(['usul_belanja_rpk.id_pkm' => $value->id_pkm, 'sub_komponen.id_rincian' => $value->id_rincian])
                ...
                foreach ($komponen as $key => $value) {
                    ...
                    foreach ($result as $row) {
                        ...
                        $isitampil .= "<td style='text-align:right;'> <b>" . number_format($row['harga_total']) . "</b></td>";
                        $isitampil .= "<td></td>";
                        $isitampil .= "</tr>";
                        ...
                        $result2 = $this->usulanrpkModel->select('usul_belanja_rpk.*, kode_belanja.nama_belanja, kode_belanja.satuan, kode_belanja.harga, kode_belanja.satuan, kodrek.kode')
                            ...
                            ->where([
                                'usul_belanja_rpk.id_pkm' => $value->id_pkm,
